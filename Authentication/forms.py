...
class PatientProfile(forms.ModelForm):
    ...
    sex =  forms.ChoiceField(widget=forms.RadioSelect, choices=CHOICES, error_messages={'required':"Please select sex"})

    class Meta:
        model = Patient
        fields = ('sex','birthdate','idcard','phone','address')
        widgets = {
            'birthdate': forms.DateInput(attrs={'class': 'myDateClass','placeholder':'Birthdate'}),
